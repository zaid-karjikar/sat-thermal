...
The default `ra=0, dec=0` places the Sun on the $+x$ axis (at the vernal equinox), corresponding approximately to the March equinox. This is a convenient reference point for tests but is only physically accurate for simulations starting around that date.

### 4.3 Satellite position in ECI
